...
(() => {
  ...
  function mount(root, config = {}) {
    ...
    const cards = Array.from({ length: count }, (_, i) => {
      ...
      const cardRatio = ({ '4:3': 4/3, '1:1': 1, '3:4': 3/4, '9:16': 9/16 })[item.cardRatio] || 4/3;
    ...
    function draw() {
      ...
      for (const card of cards) {
        ...
        const perspective = 1 / (1 - z * 0.20);
        const scale = (0.76 + (z + 1) * 0.16) * perspective;
        card.depth = z;
        const spread = 1 + (1 - reveal) * 2.6;
    ...
    function resize() {
      ...
      cardWidth = Math.min(140, span * 0.215);
    ...
    function tick(time) {
      ...
      if (!drag) yaw += dt * speed * (hover ? 0.3 : 1);
